...
st.write("이 이미지 분류기는 koala,menti분류기에요")
st.image(["koala.jpg","menti.jpg"], caption=["koala","menti"], width=300)
#use_column_width=True      

# 모델 및 클래스 불러오기
@st.cache_resource
def load_model_and_labels():
    model_path = hf_hub_download(repo_id="ktk21705/koala_manti_test", filename="EffiB0_test.h5")
    label_path = hf_hub_download(repo_id="ktk21705/koala_manti_test", filename="EffiB0_test.json")
...
if uploaded_file is not None:
    ...
    if max_probability < 0.6:
        st.markdown("### ❌ 예측 실패")
        st.markdown("신뢰도가 낮습니다. 더 명확한 이미지를 업로드해주세요.")
    else:
